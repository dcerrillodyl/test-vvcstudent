...
void Actor::animate() {
    ...
    Sprite::animate();
    advance_frame(1);
    if(current_frame() > m_animation_cells -1)
        //then
        current_frame(0);
    if(animating() && is_moving()) {
        switch(direction()) {
            case SPRITE_DOWN:
                this->setPos(this->pos()+QPointF(0,advance_distance()));
                break;
            case SPRITE_UP:
                this->setPos(this->pos()+QPointF(0,-1*advance_distance()));
                break;
            case SPRITE_LEFT:
                this->setPos(this->pos()+QPointF(-1 * advance_distance(),0));
                break;
            case SPRITE_RIGHT:
                this->setPos(this->pos()+QPointF(advance_distance(),0));
                break;
        }
